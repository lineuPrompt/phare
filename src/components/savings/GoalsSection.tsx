...
// The Goals half of /savings (2026-08-08). Lifted VERBATIM out of the old
// /goals page — same fetch, same state, same cards, same history/upcoming
// lists, same two modals. The only edits are structural: the page shell
// (Navbar/Sidebar/<main>) moved up to the route, `locale` arrives as a prop
// instead of being re-derived from the pathname, and the old <h1> is now the
// section's <h2>.
//
// One real fix carried by the move: the goal's target-date line used to
// render the SIDEBAR NAV LABEL as its field label (tDash('nav.goals'), i.e.
// "Goals: October 2026"). With the two nav items collapsed into one, that
// label would have read "Savings & Reserves: October 2026". It now has its
// own key, goals.targetDate, with the colon inside the string so French
// keeps its " : " spacing convention.
      ...
      {showCreate && (
        <div className="rounded-2xl bg-white p-6" style={{ border: '1px solid #E5E7EB' }}>
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-bold" style={{ color: '#0F2044' }}>
              {t('create.title')}
            </h3>
