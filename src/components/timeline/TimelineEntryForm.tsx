...
export default function TimelineEntryForm({
  ...
  const canSave = isTransfer
    ? parseFloat(amount) > 0 && (destinationAccountId || goalAccounts.length > 0)
  ...
  return (
    <div className="rounded-2xl bg-white p-6" style={{ border: '1px solid #E5E7EB' }}>
      ...
      {/* Money-in / Money-out / Transfer toggle */}
      <div className="flex gap-2 mb-4">
        {(['expense', 'income', 'transfer'] as const).map((tp) => (
          <button key={tp} onClick={() => switchType(tp)}
            ...
            {tp === 'expense' ? t('moneyOut') : tp === 'income' ? t('moneyIn') : t('transfer')}
          </button>
        ))}
      </div>
          ...
          <select value={destinationAccountId || goalAccounts[0]?.id || ''} onChange={(e) => setDestinationAccountId(e.target.value)}
            ...
            {goalAccounts.map((g) => (
              <option key={g.id} value={g.id}>
                {g.isDebt ? `💳 ${t('debtOption', { name: g.name })}` : `🎯 ${g.name}`}
              </option>
            ))}
          </select>
      ...
      {/* Once / Recurring toggle — recurring routes through the real recurring-item
          ...
          Not shown for transfer: a one-off contribution/payment only here —
          recurring ones are set up from the Goals or Recurring page. */}
      {!isTransfer && (
