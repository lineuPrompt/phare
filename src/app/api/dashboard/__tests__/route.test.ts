...
//
// The route no longer wires monthly_goals/statement-cycle actuals into a
// response field here — the single-month "Projected month-end" tile this
// used to feed was retired in favour of the chained 12-month plan on
// Timeline (planChainHelpers.ts, GET /api/timeline?includePlan=1). The
// underlying closed/open/max math (computeCardEnvelopeRemainders) is still
// covered by projectionHelpers.test.ts, unchanged and reused by that plan.
