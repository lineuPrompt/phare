...
-- Captures the known-good production state as of 2026-06-18.
...
-- One row per household member. Supports future multi-member households.
...
-- 10 seeded expense categories + income categories, bilingual, user-editable.
...
-- Every individual money movement (manual, materialized recurring, bridge).
CREATE TABLE IF NOT EXISTS transactions (
  ...
  amount                numeric(12,2) NOT NULL,
  type                  text NOT NULL CHECK (type IN ('income', 'expense')),
  source                text NOT NULL DEFAULT 'manual' CHECK (source IN ('manual', 'bridge')),
  recurrence_id         uuid,            -- client-generated; groups a manually-created monthly/installment series
  installment_label     text,            -- e.g. "1/12", "2/12"
  is_bridge             boolean NOT NULL DEFAULT false,
  bridge_source_account uuid REFERENCES accounts(id) ON DELETE SET NULL,
  bridge_source_month   text CHECK (bridge_source_month ~ '^\d{4}-\d{2}$'),  -- YYYY-MM
  created_at            timestamptz NOT NULL DEFAULT now(),
  updated_at            timestamptz NOT NULL DEFAULT now()
);

-- At most one bridge payment row per card per spending month.
CREATE UNIQUE INDEX IF NOT EXISTS transactions_bridge_unique
...
-- month is always stored as YYYY-MM-01 (first of month).
...
CREATE TABLE IF NOT EXISTS sinking_funds (
  ...
  monthly_provision numeric(12,2) NOT NULL DEFAULT 0,
...
-- Per-month card spending targets (upserted; unique on household+month).
-- month is always stored as YYYY-MM-01.
CREATE TABLE IF NOT EXISTS monthly_goals (
  id           uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  household_id uuid NOT NULL REFERENCES households(id) ON DELETE CASCADE,
  month        date NOT NULL,
  card_goal    numeric(12,2) NOT NULL,
  created_at   timestamptz NOT NULL DEFAULT now(),
  updated_at   timestamptz NOT NULL DEFAULT now(),
...
CREATE TABLE IF NOT EXISTS conversations (
  ...
  household_id uuid NOT NULL REFERENCES households(id) ON DELETE CASCADE,
  user_id      uuid NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  type         text NOT NULL CHECK (type IN ('onboarding', 'monthly_review')),
...
CREATE INDEX IF NOT EXISTS idx_transactions_recurring_item
...
CREATE INDEX IF NOT EXISTS idx_recurring_items_household
...
CREATE OR REPLACE TRIGGER set_monthly_goals_updated_at
  BEFORE UPDATE ON monthly_goals FOR EACH ROW EXECUTE FUNCTION set_updated_at();
...
-- Stable helper used by every household-scoped RLS policy.
...
-- households: caller can only see/update their own household
...
-- All remaining tables are household-scoped: full access to own household rows
...
-- On failure, the auth.users insert rolls back (SECURITY DEFINER, same txn).
-- =============================================================================
...
DECLARE
  ...
  member_name      text;
BEGIN
  member_name := COALESCE(NEW.raw_user_meta_data->>'full_name', '');

  INSERT INTO households DEFAULT VALUES
    RETURNING id INTO new_household_id;

  INSERT INTO users (id, household_id, full_name)
    VALUES (NEW.id, new_household_id, member_name);
